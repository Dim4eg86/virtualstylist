...
async def start(message: types.Message):
    ...
    welcome_text = (
        ...
        "• Работаю с передовой AI-технологией Flux-Redux\n"
        "• Создаю реалистичные примерки с сохранением деталей\n\n"
        "👕 <b>Категории одежды:</b>\n"
        "• <b>Верх</b>: рубашки, блузки, свитера, топы\n"
        "• <b>Низ</b>: брюки, юбки, шорты\n"
        "• <b>Платье</b>: цельные платья, сарафаны ✨\n\n"
        "💡 <i>Просто загрузи своё фото и фото одежды - я покажу как это будет выглядеть на тебе!</i>\n\n"
...
async def set_cat(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Специальное сообщение для платьев
    if key == "dresses":
        message_text = (
            "👗 <b>Примерка платья</b>\n\n"
            "📷 <b>Отправь фото платья</b>\n\n"
            "💡 <i>Совет: Для лучшего результата используй фото платья на белом фоне или манекене</i>\n\n"
            "✨ <i>AI создаст примерку цельного платья!</i>"
        )
    else:
        message_text = (
