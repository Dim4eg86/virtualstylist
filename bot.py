...
def get_animation_type_kb():
    """Выбор типа анимации"""
    builder = InlineKeyboardBuilder()
    builder.button(text="🔄 Поворот вокруг себя (5 сек)", callback_data="anim_turn")
...
async def start(message: types.Message):
    ...
    # URL приветственного видео из GitHub
    video_url = "https://raw.githubusercontent.com/Dim4eg86/virtualstylist/main/welcome.mp4"
    
    welcome_text = (
        "👗 <b>Твой стилист</b>\n"
        "Персональная примерка одежды по фото.\n\n"
        "Посмотри, как вещь будет сидеть на твоей фигуре —\n"
        "без примерочных, спешки и сомнений.\n\n"
        "• Реалистичная AI-примерка за 7–30 секунд\n"
        "• Точная посадка и сохранение деталей\n"
        "• Верх, низ и платья\n\n"
        "Загрузи своё фото и фото одежды —\n"
        "я покажу результат. Сделаю фото или видео!\n\n"
    ...
    try:
        ...
        print(f"DEBUG: Ошибка загрузки видео: {e}")
        # Если видео не загрузилось, отправляем просто текст
        await message.answer(welcome_text, reply_markup=get_main_menu())

# ВРЕМЕННЫЙ хэндлер для получения file_id видео
@dp.message(F.video)
async def get_video_file_id(message: types.Message):
    """Временный хэндлер - отправь видео, получишь file_id"""
    file_id = message.video.file_id
    await message.answer(
        f"📹 <b>File ID твоего видео:</b>\n\n"
        f"<code>{file_id}</code>\n\n"
        f"Скопируй этот ID и отправь мне!"
    )
...
async def start_vton(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "📸 <b>Как получить лучший результат примерки</b>\n\n"
        "Чтобы одежда выглядела максимально реалистично, обрати внимание на фото:\n\n"
        "<b>Твоё фото:</b>\n"
        "• В полный рост или по пояс\n"
        "• Прямо, без поворотов\n"
        "• Хорошее освещение, без теней\n"
        "• Однотонный фон — по возможности\n"
        "• Одежда на тебе должна быть не слишком свободной\n\n"
...
async def set_cat(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Сообщение для платьев
    if key == "dresses":
        message_text = (
            "👗 <b>Примерка платья</b>\n\n"
            "⚡ Генерация за ~7 секунд\n"
            "🎯 Замена платья на новое с сохранением деталей\n\n"
            "💰 <b>Стоимость: 80₽</b>\n\n"
            "📷 <b>Отправь фото платья</b>\n\n"
            "💡 <b>Важно для лучшего результата:</b>\n"
            "• <b>Твоё фото желательно в платье</b> (не в брюках!)\n"
            "• Фото одежды на белом фоне или манекене\n"
            "• AI заменит твоё платье на новое ✨"
        )
    else:
        message_text = (
...
async def garment_step(message: types.Message, state: FSMContext):
    ...
    if category.lower() in ["платье", "dress", "dresses"]:
        price = 8000  # 80₽ для платьев (FASHN v1.5)
        price_text = "80₽"
        model_text = "FASHN v1.5 (2026)"
        time_text = "⚡ Обычно это занимает ~7 секунд"
    else:
        price = 5000  # 50₽ для верха/низа (IDM-VTON)
        price_text = "50₽"
        model_text = "IDM-VTON"
        time_text = "⏳ Обычно это занимает 20-30 секунд"
    
    status_msg = await message.answer(
        f"✨ <b>Создаю твой образ...</b>\n\n"
        f"{time_text}\n"
        f"🎨 AI {model_text} рисует реалистичную картинку\n"
        f"💰 Стоимость: {price_text}"
    )
    
    try:
        result_url = await generate_vton_image(data['human'], garment_url, category)
        
        # Сохраняем в историю
        await db.save_generation(message.from_user.id, category, result_url)
        
        photo_res = requests.get(result_url).content
        
        # Списываем соответствующую цену
        await db.update_balance(message.from_user.id, -price, is_video=False)
        new_balance = (user['balance'] - price) / 100
        ...
        print(f"DEBUG: Списано {price_text}, новый баланс: {new_balance}₽")
...
async def video_set_cat(callback: types.CallbackQuery, state: FSMContext):
    ...
    print(f"DEBUG video_set_cat: ВЫЗВАН! Callback data = {callback.data}")
    print(f"DEBUG video_set_cat: Текущее состояние = {await state.get_state()}")
